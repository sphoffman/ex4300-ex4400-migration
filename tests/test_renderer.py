...
from test_provisioner import devices_for, digest, plan_variables, policy
...
def test_pre_stage_render_is_deterministic_and_contains_only_approved_vlan_definitions():
    ...
    rendered_a, validation_a = render_pre_stage(template, contract, package, "0.8.0")
    rendered_b, validation_b = render_pre_stage(template, contract, package, "0.8.0")
...
def test_pre_stage_static_validation_rejects_endpoint_vlan_assignment():
    ...
    template = (ROOT / "templates/ex4400/ex4400.set.j2").read_text()
    contract = json.loads((ROOT / "templates/ex4400/contract-v1.json").read_text())
    rendered, _validation = render_pre_stage(template, contract, package, "0.8.0")
...
def test_render_rejects_stale_renderer_version():
    ...
    contract = json.loads((ROOT / "templates/ex4400/contract-v1.json").read_text())
    with pytest.raises(RenderError):
        render_pre_stage(template, contract, package, "0.8.1")
...
def test_render_manifest_is_deterministic_and_schema_valid():
    ...
    template = (ROOT / "templates/ex4400/ex4400.set.j2").read_text()
    contract = json.loads((ROOT / "templates/ex4400/contract-v1.json").read_text())
    rendered, validation = render_pre_stage(template, contract, package, "0.8.0")
    package_digest = sha256_bytes(canonical_bytes(package))
    config_digest = sha256_bytes(rendered.encode("utf-8"))
    manifest_a = build_render_manifest(package, package_digest, config_digest, validation, "0.8.0")
    manifest_b = build_render_manifest(package, package_digest, config_digest, validation, "0.8.0")
