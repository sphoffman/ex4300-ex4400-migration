...
def _prompt_int(value, label, default):
    raw = _prompt(value, label, default)
    try:
        return int(raw)
    except ValueError:
...
def _site_init(argv):
    ...
    if path.is_file():
        ...
        answer = input("Replace this site profile? [y/N]: ").strip().lower()
    ...
    if excluded:
        print("  Explicitly excluded ET interfaces: %s" % ", ".join(excluded))
    else:
        print("  Explicitly excluded ET interfaces: none")

    answer = input("\nCreate this site profile? [y/N]: ").strip().lower()
    if answer not in ("y", "yes"):
...
def _site_discover(argv):
    ...
    args = parser.parse_args(argv)
    settings = load_settings(args.settings)
    profile, _path = load_profile(settings)
    ...
    if discovery["asymmetric_interfaces"]["qfx-a-only"] or discovery["asymmetric_interfaces"]["qfx-b-only"]:
        print("\nAsymmetric physical inventory")
        print("  qfx-a only: %s" % (", ".join(discovery["asymmetric_interfaces"]["qfx-a-only"]) or "none"))
        print("  qfx-b only: %s" % (", ".join(discovery["asymmetric_interfaces"]["qfx-b-only"]) or "none"))
    ...
    answer = input("Approve this discovered QFX identity and staging plan? [y/N]: ").strip().lower()
    if answer not in ("y", "yes"):
...
def _candidate_diff(cu):
    return str(cu.diff() or "")
...
def _site_stage(argv):
    ...
    settings = load_settings(args.settings)
    profile, _profile_path = load_profile(settings)
    candidates = site_discovery_candidates(settings, profile)
    if not candidates:
    ...
    try:
        ...
        discovered_by_role = {item["role"]: item for item in discovery["qfx_pair"]}
        for role in ("qfx-a", "qfx-b"):
            observed = observed_by_role[role]
            bound = discovered_by_role[role]
            if (
                observed["hostname"] != bound["hostname"]
                or observed["model"] != bound["model"]
                or observed["serial_number"] != bound["serial_number"]
                or observed["ssh_host_key_sha256"] != bound["ssh_host_key_sha256"]
            ):
                raise SiteError("%s identity changed since approved site discovery" % role)
        ...
        if diffs["qfx-a"] or diffs["qfx-b"]:
            ...
            print("\nIntent: every approved ET attachment maps to its AE and carries ONLY the management and TEMP-RECOVERY baseline VLANs. No voice or endpoint data VLAN is added here.")
            answer = input("\nApprove exactly these site baseline candidates for commit confirmed on both QFXs? [y/N]: ").strip().lower()
            if answer not in ("y", "yes"):
